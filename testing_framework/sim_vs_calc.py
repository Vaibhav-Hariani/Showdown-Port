...
def assert_results(sim_damage_scaled, calc_damages, sim_status_after, 
                   sim_boosts_after, player_name, original_cur_hp, calc_result):
    actual_damage = round(sim_damage_scaled * original_cur_hp / 1000)
    assert actual_damage in calc_damages, \
    ...
    assert not non_neutral, \
        f"{player_name}: Unexpected boost changes: {non_neutral}"
...
def choose_random_move(active_pokemon):
    ...
    valid = [(i, m.get("name")) for i, m in enumerate(moves[:4]) 
...
def test_simulator_vs_calculator():
    ...
    print(f"\n=== SIMULATOR RESULTS ===")
    print(f"P1: HP {hp_before_p1}/1000 -> {hp_after_p1}/1000 (scaled damage: {sim_damage_p1})")
    print(f"P2: HP {hp_before_p2}/1000 -> {hp_after_p2}/1000 (scaled damage: {sim_damage_p2})")
    ...
    with DamageCalculator() as calc:
        ...
        result_p2 = calc.calculate(gen=1, attacker=attacker_p2, defender=defender_p1, move=move_p2)
        
        calc_damages_p1 = result_p2.damage
        
        print(f"\n=== CALCULATOR RESULTS (P2 -> P1) ===")
        print(f"Move: {move_name_p2}")
        print(f"Damage range: {result_p2.min_damage()} - {result_p2.max_damage()} (avg {result_p2.average_damage():.1f})")
        ...
        if not p2_fainted:
            assert_results(
                sim_damage_p2, calc_damages_p2, status_after_p2,
                boosts_after_p2, "P2 (from P1's attack)", result_p1.originalCurHP, result_p1
            )
        ...
        if not p1_fainted:
            assert_results(
                sim_damage_p1, calc_damages_p1, status_after_p1,
                boosts_after_p1, "P1 (from P2's attack)", result_p2.originalCurHP, result_p2
            )
