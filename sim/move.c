...
inline void reset_action(Action* a) { memset(a, 0, sizeof(Action)); }

inline action_union get_action_action_d(Action* a) { return a->action_d; }
inline void set_action_action_d(Action* a, action_union d) { a->action_d = d; }

inline action_types get_action_action_type(Action* a) { return a->action_type; }
inline void set_action_action_type(Action* a, action_types type) {
  a->action_type = type;
}

inline Player* get_action_user(Action* a) { return a->User; }
inline void set_action_user(Action* a, Player* user) { a->User = user; }

inline Player* get_action_target(Action* a) { return a->Target; }
inline void set_action_target(Action* a, Player* target) { a->Target = target; }

inline int get_action_order(Action* a) { return a->order; }
inline void set_action_order(Action* a, int order) { a->order = order; }

inline int get_action_priority(Action* a) { return a->priority; }
inline void set_action_priority(Action* a, int priority) {
  a->priority = priority;
}

inline int get_action_speed(Action* a) { return a->speed; }
inline void set_action_speed(Action* a, int speed) { a->speed = speed; }

inline int get_action_orig_loc(Action* a) { return a->origLoc; }
inline void set_action_orig_loc(Action* a, int origLoc) {
  a->origLoc = origLoc;
}

inline size_t get_action_size() { return sizeof(Action); }