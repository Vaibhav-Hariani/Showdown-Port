...
// Cannot see entire team: only pokemon they played already. This can be simulated with missingNo for unknowns? 
...
struct STR_BATTLE {
  ...
  //Major modification happening here by moving Move actions outside of a BattleActions class
  //This should streamline things (hopefully)?  
  int turn_num;
  ...
  //It seems the architecture relies on these being set, and then handles poison/toxic and other callback loops
  //Leaving them out for now, but may need them in the near future 
